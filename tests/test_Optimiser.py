...
class OptimiserTest(unittest.TestCase):
  ...
  def setUp(self):
    self.optimiser = sslouvain.Optimiser();

  def test_move_nodes(self):
    G = ig.Graph.Full(100);
    partition = sslouvain.CPMVertexPartition(G, resolution_parameter=0.5);
    self.optimiser.move_nodes(partition, consider_comms=sslouvain.ALL_NEIGH_COMMS);
  ...
  def test_diff_move_node_optimality(self):
    ...
    partition = sslouvain.CPMVertexPartition(G, resolution_parameter=0.1);
    while 0 < self.optimiser.move_nodes(partition, consider_comms=sslouvain.ALL_NEIGH_COMMS):
      pass;
    for v in G.vs:
  ...
  def test_optimiser(self):
    ...
    partition = sslouvain.CPMVertexPartition(G, resolution_parameter=0);
    self.optimiser.consider_comms=sslouvain.ALL_NEIGH_COMMS;
  ...
  def test_neg_weight_bipartite(self):
    ...
    G.es['weight'] = -0.5;
    partition = sslouvain.CPMVertexPartition(G, resolution_parameter=-0.5, weights='weight');
    self.optimiser.consider_comms=sslouvain.ALL_COMMS;
    self.optimiser.optimise_partition(partition);
    self.assertListEqual(
  ...
  def test_resolution_profile(self):
    ...
    profile = self.optimiser.resolution_profile(G, sslouvain.CPMVertexPartition, resolution_range=(0,1));
    self.assertListEqual(
